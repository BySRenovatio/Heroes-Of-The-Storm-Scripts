...
Func Go_Attack()
   ...
   While $exitBut <> 11379454
	  ...
	  If $damage <> 3908373 And $damage <> 3842325 Then									;;;; 3908373  tracer/samuro/d.va-3842325
		 $forVal = Random(8, 12, 1)
		 For $i = 1 To $forVal Step +1
			$checkDead = PixelGetColor($Xx + 940, $Yy + 800)
			If $checkDead = 856088 Then
...
Func Use_Ability()
   ...
   If 1 <= $abl And $abl <= $zmax And (Int($check/100000) = 26) Then			;;; z
	  Send("z")
	  Sleep(1000)																;;; medivh
	  Return
   EndIf

   If $zmax+1 <= $abl And $abl <= $zmax+$qmax And (Int($check/100000) <> 158) Then															;;; q
	  Send("q")
	  MouseClick("left", $Xx + $xMid + RandySide() * Random($xmin, $xmax, 1), $Yy + $yMid + RandySide() * Random($ymin, $ymax, 1), 1, 0)
	  Return
   EndIf

   If $zmax+$qmax+1 <= $abl And $abl <= $zmax+$qmax+$wmax And (Int($check/100000) <> 158) Then												;;; w
	  Send("w")
	  MouseClick("left", $Xx + $xMid + RandySide() * Random($xmin, $xmax, 1), $Yy + $yMid + RandySide() * Random($ymin, $ymax, 1), 1, 0)
	  Return
   EndIf

   If $zmax+$qmax+$wmax+1 <= $abl And $abl <= $zmax+$qmax+$wmax+$emax And (Int($check/100000) <> 158) Then									;;; e
	  Send("e")
	  MouseClick("left", $Xx + $xMid + RandySide() * Random($xmin, $xmax, 1), $Yy + $yMid + RandySide() * Random($ymin, $ymax, 1), 1, 0)
	  Return
   EndIf

   If $zmax+$qmax+$wmax+$emax+1 <= $abl And $abl <= $zmax+$qmax+$wmax+$emax+$rmax And (Int($check/100000) <> 158) Then						;;; r
	  Send("r")
	  MouseClick("left", $Xx + $xMid + RandySide() * Random($xmin, $xmax, 1), $Yy + $yMid + RandySide() * Random($ymin, $ymax, 1), 1, 0)
	  Return
   EndIf
